...
def _detect_orm() -> Optional[str]:
    """Detect ORM from requirements.txt by matching known package names exactly."""
    ...
    for line in req_file.read_text().splitlines():
        ...
        if not line or line.startswith("#"):
            continue
        # 取包名：去掉 extras([...])、版本说明符、环境标记(;)
        match = re.match(r"^([A-Za-z0-9]([A-Za-z0-9._-]*[A-Za-z0-9])?)", line)
        if match:
            packages.add(match.group(1).lower())

    has_sqlalchemy = "sqlalchemy" in packages
    has_tortoise = "tortoise-orm" in packages

    if has_sqlalchemy and has_tortoise:
        return None  # 无法确定，让用户显式指定
    if has_sqlalchemy:
        return "sqlalchemy"
    if has_tortoise:
        return "tortoise"
...
def models(
    ...
    if orm is None:
        ...
        if orm:
            ...
            console.print(
                "[red]Error: Could not detect ORM from requirements.txt. "
                "Please specify --orm tortoise or --orm sqlalchemy.[/red]"
            )
            raise typer.Exit(1)
    ...
    try:
        with MySQLIntrospector(db_url) as introspector:
            table_infos = introspector.get_tables(table_list)

        if not table_infos:
            console.print("[yellow]No tables found. Check that the database URL and table names are correct.[/yellow]")
            raise typer.Exit(1)
